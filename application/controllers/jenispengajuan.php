<?php
class jenispengajuan extends CI_Controller{
    
    var $folder =   "jenispengajuan";
    var $tables =   "pengajuan_jenis_surat";
    var $pk     =   "jenis_bayar_id";
    var $title  =   "Jenis Pengajuan Surat";
	...
	$this->table->set_heading($nomber,'Jenis Pengajuan',$option);
	foreach ($record as $r) 
            {
                $pk=  $this->pk;
    ...
    function post()
    {
        if(isset($_POST['submit']))
        {
            ...
            $jns_byr=   $this->db->get_where('pengajuan_jenis_surat',array('keterangan'=>$jenis))->row_array();
            ...
            foreach ($konsen as $k)
            {
                ...
                $this->db->insert('pengajuan_surat_kuliah',$data);
            }
