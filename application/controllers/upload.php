...
class upload extends CI_Controller
{
    ...
    var $folder =   "Upload";
    var $tables =   "pengajuan_surat_detail";
    var $pk     =   "pembayara_detail_id";
    ...
    function index()
    {
        ...
        $query="SELECT sm.nama,sm.nim,jb.keterangan,pd.tanggal,pd.jumlah,pd.semester,ak.nama_konsentrasi
                FROM pengajuan_surat_detail as pd,pengajuan_jenis_surat as jb,student_mahasiswa as sm,akademik_konsentrasi as ak
        ...
        $this->template->load('template', 'upload/view',$data);
    }
    ...
    function laporan()
    {
        ...
        $this->template->load('template', 'surat/laporan',$data);
    }
    
    function cetak()
    {
        $jenis="SELECT * FROM `pengajuan_jenis_surat` WHERE jenis_bayar_id not in('3') ";
        ...
        $this->load->view('surat/cetaklap',$data);
    }
    ...
    function tampilkandata2($konsentrasi,$tahun_angkatan)
    {
        ...
        $jenis_bayar=  $this->db->get('pengajuan_jenis_surat');
        echo   "<table class='table table-bordered'>
...
function simpanstatus()
    {
        $nilaidosen =   $_GET['jenis_bayar_id'];
        $this->mcrud->update($this->tables,array('status'=>$nilaidosen), $this->pk,$id);
        echo "<div class='alert alert-success'>Berhasil Diperbaharui <i class='gi gi-ok'></i> </div>"; 
    ...
    function bayar()
    {
        ...
        elseif(isset($_POST['submit2']))
        {
            ...
            // jika selain spp chek sudah lunas atau belum
             ...
             if($jenis==3)
             {
                 ...
                 else
                 {
                    ...
                    $this->db->insert('pengajuan_surat_detail',$data);
                    }
                 }
             }
             else
             {
                ...
                {
                    // sudah lunas
                     $this->session->set_flashdata('pesan', "<div class='alert alert-danger'><i class='fa fa-bullhorn'></i> PEMBAYARAN <b> ".  strtoupper(getField('pengajuan_jenis_surat', 'keterangan', 'jenis_bayar_id', $jenis))." </b> OK </div>");
                }
                elseif($jumlah>$sisa)
                {
            ...
            redirect('surat/bayar');
        }
        else
        {
            $nim_session=$this->session->userdata('pembayaran_mahasiswa_nim');
            $query2="   SELECT au.nama,kj.keterangan,kd.tanggal,kd.jumlah,kd.pembayara_detail_id,kd.jenis_bayar_id,kd.semester,kd.status
                        FROM  pengajuan_surat_detail  as kd,pengajuan_jenis_surat as kj,app_users as au
            ...
            $data['jenis_bayar']=  $this->db->query('select * from pengajuan_jenis_surat where jenis_bayar_id not in("3")')->result();
            $data['nim']=$nim_session;
            $data['semester']=getField('student_mahasiswa', 'semester', 'nim', $nim_session);
            $data['title']=  $this->title;
            $this->template->load('template', 'upload/bayar/view',$data);
        }
    }
...
function clear()
    ...
    function cetakpersonal()
    {
        ...
        else
        {
           ...
           $this->load->view('surat/cetakpersonal',$data); 
        }
    }
    ...
    function laporanpembayaran()
    {
        ...
        $query="SELECT sm.nama,sm.nim,jb.keterangan,pd.tanggal,pd.jumlah,pd.semester,pd.status,ak.nama_konsentrasi
                FROM pengajuan_surat_detail as pd,pengajuan_jenis_surat as jb,student_mahasiswa as sm,akademik_konsentrasi as ak
        ...
        $data['transaksi']=  $this->db->query($query)->result();
        $this->load->view('surat/laporanpembayaran',$data);
    }
    ...
    function viewsurat()
    {
        if(isset($_POST['submit']))
        {
			...
			$data['tanggal1']=$this->input->post('tanggal1');
            $data['tanggal2']=$this->input->post('tanggal2');
            $query="SELECT sm.nim,sm.nama,jb.keterangan,pd.tanggal,pd.jenis_bayar_id,pd.jumlah,pd.pembayara_detail_id,pd.status,pd.semester
                    FROM pengajuan_surat_detail as pd,student_mahasiswa as sm,pengajuan_jenis_surat as jb
    ...
    function cetakjurnalkhusus()
    {
        ...
        $data['transaksi']=  $this->db->query($query)->result();
        $this->load->view('surat/cetakjurnalkhusus',$data);
    }
        ...
        function delete()
    {
         ...
         $this->mcrud->delete('pengajuan_surat_detail',  'pembayara_detail_id',  $id);
         redirect('surat/pembayaran');
    }
	 function edit()
    {
         ...
         $this->mcrud->update('pengajuan_surat_detail',array('status'=>FINISH),'pembayara_detail_id',$id);
         redirect('surat/viewsurat'); 
		
    }
    
    function proses_detail_id()
	{
	}
        ...
        foreach ($tahun_akademik as $t)
        {
            ...
            foreach ($konsentrasi as $k)
            {
                ...
                foreach ($mahasiswa as $m)
                {
                    ...
                    $tunggakan=0;
                    $jenis_bayar=  $this->db->get('pengajuan_jenis_surat')->result();
